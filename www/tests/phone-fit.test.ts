...
describe('CSS mirror constants', () => {
  ...
  /* .demo-play's vertical-align in opportunity.css seats the deck's play
     button on the slide title's baseline with a calc() whose four literals
     (see that rule's own long comment) are copies of values that live in two
     other files: the button's padding-bottom and type size in globals.css,
     and the play glyph's SVG height in DemoControls.tsx. Nothing but this
     test ties the copies back to their sources — change any of the four at
     its source and the calc silently goes stale. */
  it('the .demo-play vertical-align calc in opportunity.css matches its padding, type size and glyph height sources', () => {
    const demoControlsTsx = () =>
      readFileSync(
        path.join(__dirname, '..', 'components', 'demo', 'DemoControls.tsx'),
        'utf8'
      );

    const globals = globalsCss();
    const playRuleMatch = globals.match(/\.demo-play\s*\{([^}]*)\}/);
    expect(
      playRuleMatch,
      'app/globals.css: .demo-play { … } not found'
    ).not.toBeNull();
    const playRule = playRuleMatch![1];

    const paddingMatch = playRule.match(
      /padding:\s*[\d.]+px\s+[\d.]+px\s+([\d.]+)px;/
    );
    expect(
      paddingMatch,
      "app/globals.css: .demo-play's padding shorthand not found, or not in the top/right/bottom form the vertical-align calc assumes"
    ).not.toBeNull();
    const paddingBottom = Number(paddingMatch![1]);

    const fontSizeMatch = playRule.match(/font-size:\s*([\d.]+)px;/);
    expect(
      fontSizeMatch,
      "app/globals.css: .demo-play's font-size not found"
    ).not.toBeNull();
    const fontSize = Number(fontSizeMatch![1]);

    const lineHeightMatch = playRule.match(/line-height:\s*([\d.]+);/);
    expect(
      lineHeightMatch,
      "app/globals.css: .demo-play's line-height not found"
    ).not.toBeNull();
    const lineHeight = Number(lineHeightMatch![1]);

    const glyphMatch = demoControlsTsx().match(
      /function PlayGlyph\(\)[\s\S]*?height="([\d.]+)"/
    );
    expect(
      glyphMatch,
      'components/demo/DemoControls.tsx: PlayGlyph\'s <svg height="…"> not found'
    ).not.toBeNull();
    const glyphHeight = Number(glyphMatch![1]);

    const calcMatch = opportunityCss().match(
      /\.demo-play\s*\{[^}]*vertical-align:\s*calc\(\s*([\d.]+)px\s*\+\s*\(\s*([\d.]+)px\s*\*\s*([\d.]+)\s*-\s*([\d.]+)px\s*\)\s*\/\s*2\s*\)/
    );
    expect(
      calcMatch,
      'app/opportunity/opportunity.css: .demo-play { vertical-align: calc(…) } not found, or no longer in the padding-bottom + (font-size * line-height - glyph-height) / 2 form'
    ).not.toBeNull();
    const [calcPadding, calcFontSize, calcLineHeight, calcGlyphHeight] =
      calcMatch!.slice(1).map(Number);

    expect(
      calcPadding,
      `app/opportunity/opportunity.css's vertical-align calc opens with ${calcPadding}px, but app/globals.css's .demo-play padding-bottom is ${paddingBottom}px — update the calc's first term to match`
    ).toBe(paddingBottom);
    expect(
      calcFontSize,
      `app/opportunity/opportunity.css's vertical-align calc uses ${calcFontSize}px as the content line's font-size, but app/globals.css's .demo-play font-size is ${fontSize}px — update the calc to match`
    ).toBe(fontSize);
    expect(
      calcLineHeight,
      `app/opportunity/opportunity.css's vertical-align calc uses ${calcLineHeight} as the content line's line-height multiplier, but app/globals.css's .demo-play line-height is ${lineHeight} — update the calc to match`
    ).toBe(lineHeight);
    expect(
      calcGlyphHeight,
      `app/opportunity/opportunity.css's vertical-align calc uses ${calcGlyphHeight}px as the play glyph's height, but components/demo/DemoControls.tsx's PlayGlyph <svg> height is ${glyphHeight}px — update the calc's last term to match`
    ).toBe(glyphHeight);

    const impliedOffset =
      paddingBottom + (fontSize * lineHeight - glyphHeight) / 2;
    const calcOffset =
      calcPadding + (calcFontSize * calcLineHeight - calcGlyphHeight) / 2;
    expect(
      calcOffset,
      `app/opportunity/opportunity.css's vertical-align calc evaluates to ${calcOffset}px, but its own inputs (padding-bottom ${paddingBottom}px, font-size ${fontSize}px, line-height ${lineHeight}, glyph height ${glyphHeight}px, from app/globals.css and components/demo/DemoControls.tsx) imply ${impliedOffset}px — the calc's arithmetic no longer matches its stated derivation`
    ).toBeCloseTo(impliedOffset, 10);
  });
});
