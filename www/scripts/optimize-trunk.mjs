...
if (meshCount !== BODY_MAP.length) {
  throw new Error(
    `Expected exactly ${BODY_MAP.length} meshes after cleanup, found ${meshCount}`
  );
}
...
const checkScene =
  check.getRoot().getDefaultScene() ?? check.getRoot().listScenes()[0];
const names = checkScene
  .listChildren()
  .map(n => n.getName())
  .sort();
...
if (missing.length) {
  console.error(
    `FAIL: missing body nodes in output: ${missing.join(', ')} (got: ${names.join(', ')})`
  );
  process.exit(1);
}
...
if (outMeshCount !== BODY_MAP.length) {
  console.error(
    `FAIL: expected exactly ${BODY_MAP.length} meshes in output document, found ${outMeshCount} (orphaned geometry?)`
  );
  process.exit(1);
}
...
console.log(
  `size:       ${(bytes / 1024 / 1024).toFixed(2)} MB (budget 4.00 MB)`
);
console.log(
  `triangles:  ${Math.round(tris).toLocaleString()} (budget 300,000)`
);
console.log(`draw calls: ${drawCalls}`);
if (bytes > SIZE_BUDGET_BYTES || tris > TRI_BUDGET) {
  console.error(
    'FAIL: over budget. Increase simplify() aggressiveness (lower ratio, raise error) and re-run.'
  );
  process.exit(1);
}
